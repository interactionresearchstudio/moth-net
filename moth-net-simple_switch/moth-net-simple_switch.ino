...
void loop() {
  ...
  if (millis() - prevMillisSensorPoll > SENSOR_POLL) {
    ...
    currTouch = digitalRead(SWITCH);
    if (currTouch == 0 && isTouched == false) {
      isTouched = true;
      digitalWrite(LED, LED_ON);
      Serial.println("touch triggered");

      //set sensor struct
      outgoingReadings.sensors = simple_switch;
      outgoingReadings.ID = MY_ID;
      outgoingReadings.eventVal = 1;
      sendESPNOW();
    } else if (currTouch == 1 && isTouched == true) {
      isTouched = false;
      //set sensor struct
      outgoingReadings.sensors = simple_switch;
      outgoingReadings.ID = MY_ID;
      outgoingReadings.eventVal = 0;
      digitalWrite(LED, LED_OFF);
      sendESPNOW();
    } else if (currTouch == 1 && isTouched == false) {
